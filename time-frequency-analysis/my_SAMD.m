function [f_est,modos_est,b_e] = my_SAMD(f,A,phi,D,order_Phi,options)
% Adapted from  the Shape-adaptive mode decomposition method,
...
C = zeros(2*sum(D),N);
for i = 1:I
    C(2*sum(D(1:i-1))+1:2*sum(D(1:i)),:) = construct_dct(A(i,:),phi(i,:)/(2*pi),D(i))';
end
